...
/**
 * <p>
 * This comment processor is used to parse and format comments spanning multiple lines. It is a low-level class that can be used to
 * implement higher order comment formatting strategies.
 * </p>
 * <p>
 * Comments may be a sequence of single line comments, or a multi line comment. When processing a sequence of single line comments,
 * everything in the passed region to format must be comment text or whitespace - input in this example:
 *
 * <pre>
 *  # comment
 *  a = 10 # comment
 ...
 * </pre>
 * <p>
 * <b>Usage:</b> An instance of the class is created with an ICommentContext that describes how the comment should be parsed. This context
 * includes the relative starting position of the comment sequence. One of the classes implementing this interface may be used as a
 * convenience. As an example, given the text:
 * </p>
 *
 * <pre>
 * {@code
 ...
 * </pre>
 * <p>
 * The comment processor has special processing of comment lines that consists of the same repeated character. All such lines are formatted
 * without a left margin, and such lines longer than 5 characters are truncated instead of folded/wrapped when not fitting within the max
 * constraints.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Note that the result will contain leading whitespace up to the position passed as the starting position. When appending the text to an
 * existing flow already at this position, the method {@link CharSequences#trimLeft(CharSequence)} can be used to adjust the text to this
 * position.
 * </p>
...
public class CommentProcessor {
	...
	public static class CommentFormattingOptions {
		...
		public CommentFormattingOptions(ICommentFormatterAdvice advice, int maxWidth, int minEmptyTrailing, int maxEmptyTrailing) {
			this(advice, maxWidth, minEmptyTrailing, maxEmptyTrailing, true);
		}

		public CommentFormattingOptions(ICommentFormatterAdvice advice, int maxWidth, int minEmptyTrailing, int maxEmptyTrailing,
				boolean retainInline) {
	...
	protected TextFlow emit(CommentText commentText, ICommentContainerInformation out, CommentFormattingOptions options,
	...
	public TextFlow formatComment(CommentText commentText, ICommentContainerInformation out, CommentFormattingOptions options,
			IFormattingContext context) {
		foldLines(commentText.lines, options.advice, options.getMaxWidth() - out.getMarkerColumnWidth() - out.getLeftMargin());
	 ...
	 */
	public CommentText separateCommentFromContainer(CharSequence s, ICommentContainerInformation in, String lineSeparator) {
