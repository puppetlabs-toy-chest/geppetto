...
public class SettingsData {
	...
	public final Setting[] settings = new Setting[] {
		...
		setting("ca_days", "How long a certificate should be valid. This parameter is deprecated, use ca_ttl instead.", "", true), //
		...
		setting(
			"cacrl", "The certificate revocation list (CRL) for the CA. Will be used if present but otherwise ignored.",
		...
		setting("client_datadir", "The directory in which serialized data is stored on the client.", "Default: $vardir/client_data"), //
		...
		setting("couchdb_url", "The url where the puppet couchdb database will be created", "Default: http://127.0.0.1:5984/puppet"), //
		...
		setting(
			"dblocation", "The database cache for client configurations. Used for querying within the language.",
		...
		setting("dbpassword", "The database password for caching. Only used when networked databases are used.", "Default: puppet"), //

		setting("dbport", "The database password for caching. Only used when networked databases are used.", ""), //

		setting("dbserver", "The database server for caching. Only used when networked databases are used.", "Default: localhost"), //
		...
		setting("diff_args", "Which arguments to pass to the diff command when printing differences between files.", "Default: -u"), //
		...
		setting(
			...
			"Default: none"), //
		setting("factdest", "Where Puppet should store facts that it pulls down from the central server.", "Default: $vardir/facts/"), //
		setting(
			"factpath", "Where Puppet should look for facts. Multiple directories should be colon-separated, like normal PATH variables.",
		...
		setting(
			"hostcrl", "Where the host’s certificate revocation list can be found. This is distinct from the certificate authority’s CRL.",
			"Default: $ssldir/crl.pem"), //
		setting("hostcsr", "Where individual hosts store and look for their certificate requests.", "Default: $ssldir/csr_$certname.pem"), //
		setting("hostprivkey", "Where individual hosts store and look for their private key.", "Default: $privatekeydir/$certname.pem"), //
		setting("hostpubkey", "Where individual hosts store and look for their public key.", "Default: $publickeydir/$certname.pem"), //
		setting(
			"http_compression",
		...
		setting(
			...
			"Default: false"), //
		setting(
			"ignoreschedules", "Boolean; whether puppet agent should ignore schedules. This is useful for initial puppet agent runs.",
			"Default: false"), //
		...
		setting(
			"lastrunfile", "Where puppet agent stores the last run report summary in yaml format.",
			"Default: $statedir/last_run_summary.yaml"), //
		setting("lastrunreport", "Where puppet agent stores the last run report in yaml format.", "Default: $statedir/last_run_report.yaml"), //
		setting(
			"ldapattrs",
		...
		setting(
			...
			""), //
		setting(
			"ldapclassattrs", "The LDAP attributes to use to define Puppet classes. Values should be comma-separated.",
			"Default: puppetclass"), //
		setting(
			"ldapnodes",
		...
		setting(
			"manage_internal_file_permissions", "Whether Puppet should manage the owner, group, and mode of files it uses internally",
			"Default: true"), //
		...
		setting(
			"masterlog", "Where puppet master logs. This is generally not used, since syslog is the default log destination.",
		...
		setting(
			"modulepath", "The search path for modules as a list of directories separated by the ‘:’ character.",
			"Default: $confdir/modules:/usr/share/puppet/modules"), //
		setting(
			"name", "The name of the application, if we are running as one. The default is essentially $0 without the path or .rb.",
			"Default: doc"), //
		setting(
			"node_name",
		...
		setting(
			...
			"Default: false"), //
		setting(
			"passfile", "Where puppet agent stores the password for its private key. Generally unused.", "Default: $privatedir/password"), //
		...
		setting("plugindest", "Where Puppet should store plugins that it pulls down from the central server.", "Default: $libdir"), //
		...
		setting("puppetdlockfile", "A lock file to temporarily stop puppet agent from doing anything.", "Default: $statedir/puppetdlock"), //
		...
		setting(
			"reportdir", "The directory in which to store reports received from the client. Each client gets a separate subdirectory.",
		...
		setting(
			...
			"Default: store"), //
		setting(
			"reportserver", "(Deprecated for ‘report_server’) The server to which to send transaction reports.", "Default: $server", true), //
		setting("reporturl", "The URL used by the http reports processor to send reports", "Default: http://localhost:3000/reports/upload"), //
		...
		setting(
			"resourcefile", "The file in which puppet agent stores a list of the resources associated with the retrieved configuration.",
		...
		setting(
			...
			"Default: $vardir/rrd"), //
		setting(
			"rrdinterval", "How often RRD should expect data. This should match how often the hosts report back to the server.",
			"Default: $runinterval"), //
		...
		setting(
			"server_datadir", "The directory in which serialized data is stored, usually in a subdirectory.",
		...
		setting(
			"splaylimit", "The maximum time to delay before runs. Defaults to being the same as the run interval.", "Default: $runinterval"), //
		setting(
			"ssl_client_header",
		...
		setting(
			...
			""), //
		setting(
			"templatedir", "Where Puppet looks for template files. Can be a list of colon-seperated directories.",
